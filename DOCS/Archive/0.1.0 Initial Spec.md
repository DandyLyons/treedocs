# Treedocs 0.1.0 Initial Spec

> Historical note: this was the original design and implementation-tracking draft for the first public `treedocs` release. It is archived for context and is not the normative specification for current behavior. Verify current behavior against the README, versioned schemas under `site/schemas/`, focused docs in `DOCS/`, and tests.
...
> Implementation note: this archived draft may differ from current behavior.
...
- ✅ The canonical schema lives at `site/schemas/0.2.0/treedocs.schema.json`.
...
1. ✅ It validates `treedocs.yaml` against `site/schemas/0.2.0/treedocs.schema.json`.
...
- ✅ Keep the result valid against `site/schemas/0.2.0/treedocs.schema.json`.
